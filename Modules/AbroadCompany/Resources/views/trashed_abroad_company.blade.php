...
<html lang="en">
                                        ...
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr style="text-transform: capitalize">
                                                    ...
                                                    <th>trashed on</th>
                                                    @if(in_array('Can view company abroad info option', auth()->user()->getUserPermisions()))
                                                    <th>OPTIONS</th>
                                                    @endif
                                                </tr>
                                            </thead>
                                            <tbody>
                                            @foreach($trashed_abroad_company as $i=>$company)
                                                <tr>
                                                @php
                                                    if( $trashed_abroad_company->currentPage() == 1){
                                                        $i = $i+1;
                                                    }else{
                                                        $i = ($i+1) + 10*($trashed_abroad_company->currentPage()-1);
                                                    }
                                                @endphp
                                                    <td>{{$i}}</td>
                                                    ...
                                                    @if(in_array('Can restore company abroad info', auth()->user()->getUserPermisions()))
                                                    <td class="text-nowrap" style="width:10px">
                                                        <a href="/abroadcompany/restore-abroadcompany/{{$company->id}}" class="btn btn-sm btn-info" data-bs-toggle="tooltip" data-original-title="Restore"> Restore</a>
                                                        @endif
                                                        @if(in_array('Can delete company abroad info permanently', auth()->user()->getUserPermisions()))
    ...
    <div class="modal-dialog modal-notify modal-info" role="document">
        ...
        <!--Header-->
        <form action="/abroadcompany/remove-abroad-company" method="get">
