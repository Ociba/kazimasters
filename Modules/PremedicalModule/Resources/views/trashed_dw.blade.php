                                        ...
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr style="text-transform: capitalize">
                                                    ...
                                                    <th>Created by</th>
                                                    @if(in_array('Can view trash option at premedical', auth()->user()->getUserPermisions()))
                                                    <th>OPTIONS</th>
                                                    @endif
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($trashed_dw as $i=>$dw_registered_today)
                                                <tr>
                                                @php
                                                ...
                                                @endphp
                                                    ...
                                                    <td><a href="{{ asset('dw_medical_reports/'.$dw_registered_today->premedical_report)}}" style="color:blue;">view premedical report</a></td>
                                                    <td>{{$dw_registered_today->updated_at}}</td>
                                                    <td>{{$dw_registered_today->name}}</td>
                                                    @if(in_array('Can restore dw at premedical', auth()->user()->getUserPermisions()))
                                                    <td class="text-nowrap" style="width:10px">
                                                        <a href="/premedicalmodule/restore-deleted-dw/{{$dw_registered_today->domestic_worker_id}}" class="btn btn-sm btn-info">Restore</a>
                                                        @endif
                                                        @if(in_array('Can delete dw permanently at premedical', auth()->user()->getUserPermisions()))
