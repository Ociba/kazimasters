                ...
                <div class="row">
                    ...
                    <div class="card">
                            <div class="card-body">
                                ...
                                <div class=" col-lg-4">
                                <form action="/trainingmodule/search-dw" method="get">
                                  @csrf
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach($get_dw_for_training as $i=>$training)
                                            <tr>
                                                @php
                                                    if( $get_dw_for_training->currentPage() == 1){
                                                        $i = $i+1;
                                                    }else{
                                                        $i = ($i+1) + 10*($get_dw_for_training->currentPage()-1);
                                                    }
                                                @endphp
                                                <td>{{$i}}</td>
                                                <td>{{$training->dw_last_name}} {{$training->dw_other_name}} {{$training->dw_first_name}}</td>
                                                <td>{{$training->dw_contact}}</td>
                                                <td>{{$training->dw_nin}}</td>
                                                <td>{{$training->parent_nin}}</td>
                                                <td>{{$training->nok_nin}}</td>
                                                <td>{{$training->nok_contact}}</td>
                                                <td><a href="{{ asset('dw_cvs/'.$training->dw_cv)}}" style="color:blue;" target="_blank">view cv</a></td>
                                                <td><a href="{{ asset('dw_passport/'.$training->passport)}}" style="color:blue;" target="_blank">view Passport</td>
